<template>
  <div class="min-h-screen bg-background-light bg-background-dark font-display">
    <!-- TopAppBar -->
    <header
      class="flex items-center bg-background-light bg-background-dark p-4 pb-2 justify-between sticky top-0 z-10 border-b border-white/10"
    >
      <button
        class="text-text-primary-light text-text-primary-dark flex size-10 shrink-0 items-center justify-center rounded-full hover:bg-black/5 hover:/5 transition-colors"
        @click="$router.back()"
      >
        <span class="material-symbols-outlined text-2xl">arrow_back</span>
      </button>
      <h1
        class="text-text-primary-light text-text-primary-dark text-lg font-bold leading-tight tracking-[-0.015em] flex-1 text-center"
      >
        My Subscription
      </h1>
      <div class="w-10"></div>
    ...
    <main class="flex-grow p-4">
      <!-- Plan Summary Card -->
      <section
        class="flex flex-col items-stretch justify-start rounded-xl bg-card-light bg-card-dark shadow-sm"
      >
        <div class="flex w-full min-w-72 grow flex-col items-stretch justify-center gap-2 p-6">
          <div class="flex items-start justify-between">
            <div>
              <p
                class="text-text-primary-light text-text-primary-dark text-xl font-bold leading-tight tracking-[-0.015em]"
              >
                {{ subscription.planName }}
              </p>
              <p
                class="text-text-secondary-light text-text-secondary-dark text-base font-normal leading-normal mt-1"
              >
                {{ subscription.price }}/month
              </p>
            </div>
            <div
              class="flex h-8 shrink-0 items-center justify-center gap-x-2 rounded-full bg-success/10 bg-success/20 px-3 py-1"
            >
      ...
      <section class="mt-8">
        <h2
          class="text-text-primary-light text-text-primary-dark text-lg font-bold leading-tight tracking-[-0.015em] px-2 pb-2"
        >
          Billing Details
        </h2>
        <div class="bg-card-light bg-card-dark rounded-xl shadow-sm p-4">
          <div
            class="flex justify-between gap-x-6 py-3 border-b border-black/5 border-white/10"
          >
            <p
              class="text-text-secondary-light text-text-secondary-dark text-sm font-normal leading-normal"
            >
              Next Billing Date
            </p>
            <p
              class="text-text-primary-light text-text-primary-dark text-sm font-medium leading-normal text-right"
            >
      ...
      <footer class="mt-8 text-center">
        ...
        <p class="text-text-secondary-light text-text-secondary-dark text-xs mt-6 px-4">
    ...
    >
      <div class="w-full max-w-sm rounded-xl bg-card-light bg-card-dark p-6 shadow-xl">
        <h3 class="text-lg font-bold text-text-primary-light text-text-primary-dark">
          Cancel Subscription
        </h3>
        <p class="mt-2 text-sm text-text-secondary-light text-text-secondary-dark">
          Are you sure you want to cancel your {{ subscription.planName }} plan? This action cannot
          be undone.
        </p>
        <div class="mt-6 flex justify-end gap-4">
          <button
            class="rounded-lg px-4 py-2 text-sm font-bold text-text-secondary-light text-text-secondary-dark hover:bg-black/5 hover:/10"

<template>
  <div
    ...
    style="background-color: #0a1116"
  >
    <!-- TopAppBar -->
    <div
      class="flex items-center p-4 pb-2 justify-between sticky top-0 z-10 border-b border-white/5"
      style="background-color: #0a1116"
    >
      <button
        @click="goBack"
    ...
    <main class="p-4 space-y-8">
      ...
      <section>
        ...
        <div class="rounded-xl shadow-sm" style="background-color: #111c22">
      ...
      <section>
        ...
        <div class="rounded-xl shadow-sm p-4 space-y-4" style="background-color: #111c22">
          ...
          >
            ...
            <div class="max-w-md">
              ...
              <div class="bg-white/50 dark:bg-black/20 rounded-lg p-3 text-left space-y-2">
                ...
                <div class="flex items-start gap-2">
                  <span class="material-symbols-outlined text-amber-500 text-sm mt-0.5">lock</span>
                  <p class="text-slate-700 dark:text-slate-300 text-xs">
            ...
            <div class="max-w-sm">
              ...
              <div class="bg-slate-100 dark:bg-slate-800/50 rounded-lg p-3 text-left">
                <p class="text-slate-600 dark:text-slate-400 text-xs leading-relaxed">
                  💡 <strong>Tip:</strong> If you recently created or restored this database, check
                  your email for the credentials. They were sent for security purposes.
                </p>
              </div>
            </div>
          </div>
      ...
      <section class="space-y-4 pt-4">
        ...
        <button
          ...
          :disabled="loading"
          class="w-full flex items-center justify-center gap-2 h-12 px-6 font-semibold rounded-lg bg-green-500/20 text-green-600 dark:bg-green-500/20 dark:text-green-400 hover:bg-green-500/30 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
        ...
        <button
          ...
          @click="resetPassword"
          :disabled="loading"
          class="w-full flex items-center justify-center gap-2 h-12 px-6 font-semibold rounded-lg bg-[#1193d4]/20 text-[#1193d4] dark:bg-[#1193d4]/20 dark:text-[#1193d4] hover:bg-[#1193d4]/30 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
        ...
        <button
          ...
          :disabled="loading"
          class="w-full flex items-center justify-center gap-2 h-12 px-6 font-semibold rounded-lg bg-purple-500/20 text-purple-600 dark:bg-purple-500/20 dark:text-purple-400 hover:bg-purple-500/30 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
        ...
        <button
          ...
          :disabled="loading"
          class="w-full flex items-center justify-center gap-2 h-12 px-6 font-semibold rounded-lg bg-red-500/10 text-red-600 dark:bg-red-500/20 dark:text-red-400 hover:bg-red-500/20 dark:hover:bg-red-500/30 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
...
const fetchDatabaseDetails = async () => {
  ...
  try {
    ...
    console.log('🔍 Database details received from backend:', {
      id: dbData.id,
      name: dbData.name,
    ...
    console.log('💾 Database state updated to:', {
      status: database.value.status,
      deletedAt: database.value.deletedAt,
    ...
    if (realHost === 'localhost' || realHost === '127.0.0.1') {
      ...
      console.log('🔧 Host era localhost, usando IP fija:', realHost)
    }
    ...
    if (dbData.connectionString) {
      ...
      if (hostMatch && hostMatch[1] && hostMatch[1] !== 'localhost') {
        ...
        console.log('📍 Host extraído del connectionString:', realHost)
      }
    }
    ...
    if (keepEmailMessage) {
      ...
      console.log('📧 Manteniendo mensaje de nuevas credenciales enviadas por email')
